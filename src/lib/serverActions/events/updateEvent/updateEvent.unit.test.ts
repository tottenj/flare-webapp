...
jest.mock('@/lib/serverActions/events/_shared/prepareEventMutationInput', () => ({
  __esModule: true,
  default: jest.fn(),
}));
...
describe('updateEvent', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (prepareEventMutationInput as jest.Mock).mockResolvedValue({ ok: true, ctx, data: input });
  });

  it('returns prepared failure result when input preparation fails', async () => {
    const preparedFailure: ActionResult<null> = {
      ok: false,
      error: {
        code: 'INVALID_INPUT',
    ...
    (prepareEventMutationInput as jest.Mock).mockResolvedValue({
      ok: false,
    ...
    expect(result).toBe(preparedFailure);
    expect(EventService.editEvent).not.toHaveBeenCalled();
  });
